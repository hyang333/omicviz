...
#'
#' @export
readHomerDir <- function(homer_dir, p_threshold = 0.05, top_n = NULL) {
  # --- Validate inputs ---
  ...
  if (!file.exists(results_file)) {
    stop(
      "Cannot find 'knownResults.txt' in: ",
      homer_dir,
  ...
  if (!dir.exists(logo_dir)) {
    stop(
      "Cannot find 'knownResults/' subdirectory in: ",
      homer_dir,
      "\nMake sure this is a valid HOMER output directory."
    )
  }
  ...
  if (ncol(raw_data) < 9) {
    stop(
      "knownResults.txt has fewer than 9 columns. ",
  ...
  # --- Apply filters ---
  # Only keep motifs with p-value below threshold (default 0.05)
  motif_data <- motif_data[motif_data$p_value < p_threshold, ]
  ...
  for (i in seq_len(nrow(motif_data))) {
    ...
    if (file.exists(png_file)) {
      ...
      warning(sprintf(
        "Logo file not found for motif rank %d (known%d.logo.png)",
        i,
        idx
      ))
  ...
  message(sprintf(
    "Successfully parsed %d known motifs from: %s",
    nrow(motif_data),
    homer_dir
  ))
